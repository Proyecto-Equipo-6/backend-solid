 ...
 * (CU-023) requiere sesión de Administrador (id_rol = 1). La creación y
 * edición reciben la imagen del producto como multipart (campo "imagen").
 */
function createProductoRouter(productoController, autenticar, requerirAdmin) {
  ...
  router.post('/', autenticar, requerirAdmin, uploadProductoImagen, (req, res) => productoController.crear(req, res));
  router.put('/:id', autenticar, requerirAdmin, uploadProductoImagen, (req, res) => productoController.editar(req, res));
